...
let mailTransporter = nodemailer.createTransport({ 
    service: 'gmail', 
    auth: { 
        user: process.env.PsycoolMail,
        pass: process.env.PsycoolPassword
    }
}); 
  
let mailDetails = { 
    from: process.env.PsycoolMail,
    to: "", 
    subject: "null", 
    html: "null"
};
...
function job_recommend_mail(toMail, username, recommendationList) {

    if(recommendationList.length == 1) {
        var html_msg = `Hi ${username},<br/>Given the scores from all your tests, following career fields are recommended for you:<br/>1.${recommendationList[0]}<br/><br/>Regards,<br/>CASS`;
    } else if(recommendationList.length == 2) {
        var html_msg = `Hi ${username},<br/>Given the scores from all your tests, following career fields are recommended for you:<br/>1.${recommendationList[0]}<br/>2.${recommendationList[1]}<br/><br/>Regards,<br/>CASS`;
    } else {
        var html_msg = `Hi ${username},<br/>Given the scores from all your tests, following career fields are recommended for you:<br/>1.${recommendationList[0]}<br/>2.${recommendationList[1]}<br/>3.${recommendationList[2]}<br/><br/>Regards,<br/>CASS`;
    }
