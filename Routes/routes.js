...
router.post("/careerRecommendation", async function(req,res){    
    ...
    PythonShell.run('script.py', options, function (err, results) {
        if (err) {
            ...
            Email.job_recommend_mail(req.body.email, req.body.username, recommendationList);
...
//Get career recommendation result if already generated by user
router.post("/getRecommendationResult", function(req,res) {
    let username = req.body.username;

    User.findOne({ username: username }, function (err, foundEntry) {
        if (err) {
            console.log(err);
            res.send(err);
        } else {
            if (foundEntry.recommendation == null) {
                res.send([]);
            } else {
                res.send(foundEntry.recommendation);
            }
        }
    });
});
